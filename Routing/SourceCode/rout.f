...
c     Code first developed by the University of Washington. See WA Hydrology Homepage for operational details.
c     Modified by the Resilient Water Systesm Group/Singapore University of Technology and Design to account for reservoir operations.
c     Reservoir presentation and operations are considered by the following calculation steps
c     1. We first calculate the time lag for each cell to a reservoir / the basin outlet
...
c     Make UH grid for reservoir catchments
     ...
     &        .NE.9999)) THEN		! the second condition can be removed		
          WRITE(NAMERS,*) RESER(CATCHIJ(N,1,NORESERVOIRS),
     &	    CATCHIJ(N,2,NORESERVOIRS))
     ...
     &      RES_DIRECT)
        END IF
	  END DO
c     Make UH grid for the rest of the basin to the outlet
