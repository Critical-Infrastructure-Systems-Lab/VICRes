...
C************************************************************************************************************************************************************************************
     ...
     &  XC, YC, SIZE, DPREC, INPATH,ICOL,NDAY,IDAY,IMONTH,IYEAR, START_YEAR,
...
C	  Look for starting year	  
	  IF (NO .NE. NO_STAS) THEN
		WRITE(RESNO,*) NO
		TEMPRPATH = trim(RPATH)//"RES"//trim(ADJUSTL(RESNO))//".txt"	  
		OPEN(26, FILE = TEMPRPATH,FORM = 'FORMATTED', STATUS='OLD',ERR=9002)
		READ(26,*)  
		READ(26,*) DUM1,DUM2,DUM3,DUM4,DUM5,OPEYEAR
		CLOSE(26)	  
	  END IF
...
c         WRITE(*,*) N, ILOC, JLOC
     ...
     &        AIRTEMP(I),WINDS(I) 																	! modify here accordingly depending on your selected outputs from rainfall-runoff
...
c     in input file
     ...
     &             ' NOT FOUND, INSERTING ZEROS'
		...
		! n/N is represented by nN which is the ration between actual sunshine hours and possible sunshine hours
			...
			! Ra is the solar radian (depending on location, see J.S.G. McCulloch, 1965. E. African Forest. J. (3) 286-295 for appropiate values
			  MONTH_OF_YEAR = INT(MOD(NDAY,365)/30) + 1			
     ...
     &             + 0.35*(0.5+wind/100)*(ea-ed))/(deltagamma+1)
		        ...
		        RES_EVAPORATION(NO,I) = RES_EVAPORATION(NO,I)+potentialevapo(I)*EVA_FACTOR          		       
			  END IF	
			END DO
		END IF  
     ...
     &			+ (vic_eva_soil(I)+vic_eva_vege(I)+vic_trans_vege(I))*FACTOR*0.028/1000					
		   ...
		   if (potentialevapo(I)<0.001) then
				potentialevapo(I) = 0
		   end if
...
c     Declare variables  
 ...
 400  CONTINUE 			
     ...
     &      (DIREC(II,JJ,2) .NE.0)) THEN   											!ii,jj cell routes down
				...
				IF (FINAL .EQ. 0) THEN
					TRVLTIME(RESORDER) = SIZE_CELL*1000*116/VELO(SI,SJ)				! +1 for the current cell (116km = 1 degree = 116000m)
				ELSE 
					TRVLTIME(NORESERVOIRS) = SIZE_CELL*1000*116/VELO(SI,SJ)			! +1 for the current cell (116km = 1 degree = 116000m)
				END IF
                GOTO 400
         END IF   			            											!if you get there,
      END IF                                										!no_of_box increments
 410  CONTINUE       
      TRVLTIME(RESORDER) = TRVLTIME(RESORDER)/24/3600								! convert to traveling time in days
	  RETURN
	  END
...
c     Declare variables
 ...
 310      CONTINUE                              								!and you try another
     ...
     &    .AND. (I .NE. II) .AND. (J .NE. JJ) .AND. (RESER(I,J) .NE. 9999)) THEN
				IF (RESER(PI,PJ)>0) THEN										! this part to track reservoir sequences
					DO K = 1, NORESERVOIRS
						IF (RES_DIRECT(K,1) .EQ. RESER(I,J)) THEN
							RES_DIRECT(K,2) = RESER(PI,PJ)							
						END IF
					END DO
				ELSE
					DO K = 1, NORESERVOIRS
						IF (RES_DIRECT(K,1) .EQ. RESER(I,J)) THEN
							RES_DIRECT(K,2) = NORESERVOIRS							
						END IF
					END DO
				END IF
          END IF
...
c     Declare variables
      ...
      REAL        RESFLOWS(200,DAYS) 
      REAL        HRESERMAX(200), HRESERMIN(200), VINITIAL(200)
      REAL        QRESER(200),VRESER(200),REALHEAD(200),VRESERTHAT(200)
	  ...
	  INTEGER 	  TOADOX(200), TOADOY(200), OPEYEAR(200)
	  ...
	  REAL		  SEEPAGE(200), INFILTRATION(200)
	  REAL        CRTDATE, TEMPO
      CHARACTER*20 CHUOI
	  CHARACTER*20 NAMERS	  
...
c     Look for reservoirs sequences
     ...
     &        IDAY,IMONTH,IYEAR, START_YEAR, MO, YR, NYR, VOL,
     &        FLOWIN, FLOWOUT, HHO, RESFLOWS,N,RES_EVAPORATION, NO_STAS)
      END DO
c    Initiate reservoir parameters
     ...
     & STATUS='OLD',ERR=9002)
	   READ(25,*)  
	   READ(25,*) HRESERMAX(J),HRESERMIN(J),VRESERTHAT(J),
     & HYDRAUHEAD(J), QRESERTHAT(J), OPEYEAR(J), VINITIAL(J)
       ...
       READ(25,*) RULE(J)
       IF (RULE(J) .EQ. 1) THEN
            READ(25,*) OP1(J,1),OP1(J,2), HMIN(J), HMAX(J)
			CLOSE(25)
       ELSE IF (RULE(J) .EQ. 2) THEN
            READ(25,*) X1(J), X2(J), X3(J), X4(J)
			CLOSE(25)
       ELSE         
			READ(25,'(/A)')FILENAME
			CLOSE(25)			
      ...
      DO I = 1, NDAY
       DO J = 1, NO_STAS-1
         CURRENTYEAR = START_YEAR + (START_MO-1)*30 + INT(I/365)		! approximate
         IF (CURRENTYEAR>=OPEYEAR(J)) THEN
...
c        Note RULE = 1: rule curve = 2: operating rules: pre-defined time-series data = 3
         CRTDATE = 1.0* mod(I,365)+(START_MO-1)*30
         IF (RULE(J) .EQ. 1) THEN         
     ...
     &            (HMIN(J)-HMAX(J))
		    END IF
           END IF
           HTK(J,I) = DESIGNWL + MIN(HMIN(J),HMAX(J))
           CURRENTWL = VOL(J,I) * (HRESERMAX(J)-HRESERMIN(J))/VRESER(J)
           IF (CURRENTWL>=DESIGNWL) THEN
             IF ((VOL(J,I)+FLOWIN(J,I)*24*3.6 -QRESER(J)*24*3.6)
     &     > (DESIGNWL* VRESER(J)) /(HRESERMAX(J)-HRESERMIN(J))) THEN
                VOL(J,I+1) = VOL(J,I) + FLOWIN(J,I)*24*3.6-QRESER(J)*24*3.6
                FLOWOUT(J,I) = QRESER(J)
             ELSE
                VOL(J,I+1)=(DESIGNWL*VRESER(J))/(HRESERMAX(J)-HRESERMIN(J))
                FLOWOUT(J,I)=(VOL(J,I)-VOL(J,I+1))/24/3.6 + FLOWIN(J,I)
             END IF
           GOTO 123
         ELSE
          IF ((VOL(J,I)+FLOWIN(J,I)*24*3.6)> ((DESIGNWL* VRESER(J))
     &  /(HRESERMAX(J)-HRESERMIN(J)))) THEN
     ...
     &     -HRESERMIN(J))
          ...
          END IF
         END IF
         ELSE IF (RULE(J) .EQ. 2) THEN ! opearting rule
		 ! Note x1 and x4 in radian, not degree
			IF (X2(J) .GT. QRESER(J)) THEN									! cannot discharge more than the design capacity
				X2(J) = QRESER(J)
			END IF
			IF (VOL(J,I)*tan(X1(J)) .LE. X2(J)) THEN ! hedging
				IF (tan(X1(J))*24*3.6>1) THEN								! discharge more than the water in reservoir
					FLOWOUT(J,I) = VOL(J,I)/24/3.6							! discharge all of water in the reservoir
				ELSE
					FLOWOUT(J,I) = VOL(J,I)*tan(X1(J))
				END IF
			ELSE IF (VOL(J,I) .GE. X3(J)) THEN 
				IF ((X2(J) + VOL(J,I)*tan(X4(J)))*24*3.6>VOL(J,I)) THEN		! discharge more than the water in reservoir
					FLOWOUT(J,I) = VOL(J,I)/24/3.6							! discharge all of water in the reservoir
				ELSE
					FLOWOUT(J,I) = X2(J) + VOL(J,I)*tan(X4(J))
				END IF
			ELSE ! releasing
				IF (X2(J)*24*3.6>VOL(J,I)) THEN	
					FLOWOUT(J,I) = VOL(J,I)/24/3.6
				ELSE
					FLOWOUT(J,I) = X2(J)
				END IF				
			END IF		 
			...
			GOTO 123
		 ELSE ! pre-defined time series		                 
			IF (OP2(J,I+STARTDAY(J))*24*3.6>VOL(J,I)) THEN
				IF (VOL(J,I)/24/3.6>QRESER(J)) THEN
					FLOWOUT(J,I) = QRESER(J)
				ELSE
					FLOWOUT(J,I) = VOL(J,I)/24/3.6
				END IF
			ELSE
				...
				END IF
		    END IF			
			VOL(J,I+1) = VOL(J,I+1) + (FLOWIN(J,I)-FLOWOUT(J,I))*24*3.6          
 ...
 123     END IF
         ...
         IF (FLOWOUT(J,I)<0) THEN
            FLOWOUT(J,I)=0
         END IF
         IF (VOL(I,J)<0)THEN ! Not allow dropping below the minimum water level
            VOL(I,J)=0
         END IF
...
c        Check the neccesity to spill water
         IF (VOL(J,I+1)>VRESER(J)) THEN
          ...
          VOL(J,I+1) = VRESER(J)
         END IF
         HHO(J,I)=VOL(J,I)/VRESER(J)* (HRESERMAX(J)-HRESERMIN(J))
     & + HRESERMIN(J)
         HHO(J,I+1)=VOL(J,I+1)/VRESER(J)* (HRESERMAX(J)-HRESERMIN(J))
     & + HRESERMIN(J)
     ...
     &   (( HHO(J,I)+HHO(J,I+1))/2-(HRESERMAX(J)-REALHEAD(J)))/1000									! this part is for hydropower production estimation, ignore if work with irrigation reservoirs
		 ...
		 if (RES_DIRECT(J,2) .EQ. 0) THEN
			  RESFLOWS(NO_STAS,I+INT(TRVLTIME(J))) = RESFLOWS(NO_STAS,I) + FLOWOUT(J,I)	 	 	
		 else
		      RESFLOWS(RES_DIRECT(J,2),I+INT(TRVLTIME(J)))=RESFLOWS(RES_DIRECT(J,2),I) + FLOWOUT(J,I)	 	 			 
		 end if	  		 
        ! WRITE(*,*) '-------------------------------------------------'
