...
c     Subroutine main body
 ...
 400  CONTINUE
     ...
     &      (DIREC(II,JJ,2) .NE.0)) THEN   											!ii,jj cell routes down
                ...
                IF (FINAL .EQ. 0) THEN
                    TRVLTIME(RESORDER) = SIZE_CELL*1000*116/VELO(SI,SJ)				! +1 for the current cell (116km = 1 degree = 116000m; ignore the distortion of cells in the upper and lower lattitude)
                ELSE 
                    TRVLTIME(NORESERVOIRS) = SIZE_CELL*1000*116/VELO(SI,SJ)			! +1 for the current cell (116km = 1 degree = 116000m; ignore the distortion of cells in the upper and lower lattitude)
                END IF
                GOTO 400
...
c     Declare variables
      ...
      REAL        X1(200), X2(200), X3(200)	! operating rule
...
c        Note RULE = 1: rule curve = 2: operating rules: pre-defined time-series data = 3
         CRTDATE = 1.0* mod(I,365)+(START_MO-1)*30						! appropiate
         IF (RULE(J) .EQ. 1) THEN   ! (Option 1: rule curve)
     ...
     &              (HMIN(J)-HMAX(J))
                END IF
            END IF
            HTK(J,I) = DESIGNWL + MIN(HMIN(J),HMAX(J))						! water head
     ...
     &              -HRESERMIN(J))
                ...
                END IF
            END IF
        ELSE IF (RULE(J) .EQ. 2) THEN ! opearting rule (Option 2)
        ! Note x1 and x3 in radian (0 to pi/2), not degree
            ...
            IF (VOL(J,I)*tan(X1(J)) .LE. X2(J)) THEN ! hedging
                ...
                END IF
            ELSE IF (VOL(J,I) .GE. VRESER(J)) THEN 	! spilling
                IF ((X2(J) + VOL(J,I)*tan(X3(J)))*24*3.6>VOL(J,I)) THEN		! discharge more than the water in reservoir
                    FLOWOUT(J,I) = VOL(J,I)/24/3.6							! discharge all of water in the reservoir
                ELSE
                    FLOWOUT(J,I) = X2(J) + VOL(J,I)*tan(X3(J))
                END IF
            ELSE ! releasing
