...
C************************************************************************************************************************************************************************************
     ...
     $     IROW,ICOL,NORESERVOIRS,RES_DIRECT,RESER,FINAL,SIZE_CELL,VELO,FI,FJ,NRESER_MAX)
        IMPLICIT NONE
c       Declare variables
...
C************************************************************************************************************************************************************************************
     ...
     &  XNE, YNE, NORESERVOIRS, RES_DIRECT, RES_EVAPORATION, TRVLTIME,RESORDER,NAMERS5,NAME5,NSTATIONS_MAX,UH_SS,VRESER,NRESER_MAX, NSTATIONS,STEPBYSTEP,NDAY_SIM,OUTPATH)
        IMPLICIT NONE
c       Declare variables
        INTEGER     N, DAYS, NDAY,NDAY_SIM, START_YEAR,START_MO,NSTATIONS_MAX,NRESER_MAX, NSTATIONS,PREV_DAY,PREV_MON,PREV_YEAR,STEPS,CLEN
        INTEGER     NCOL,NROW,ICOL,PMAX,KE,UH_DAY
        ...
        INTEGER     YEAR(NRESER_MAX,DAYS),MONTH(NRESER_MAX,DAYS),DAY(NRESER_MAX,DAYS)
        ...
        INTEGER     I, J, K, L, CAL_MONTH, W, x, II
        INTEGER     NO_OF_ROW(NRESER_MAX,NSTATIONS_MAX)
        INTEGER     RULE(NRESER_MAX,NSTATIONS_MAX), WITHIRRIGATION(NRESER_MAX,NSTATIONS_MAX),BTHMET_ACTIVE(NRESER_MAX,NSTATIONS_MAX)
        ...
        REAL        UH_SS(PMAX,KE+UH_DAY-1,NRESER_MAX)
        REAL        TRVLTIME(NRESER_MAX)
        ...
        REAL        VOL(NRESER_MAX,DAYS,NSTATIONS_MAX), FLOWIN(NRESER_MAX,DAYS), FLOWOUT(NRESER_MAX,DAYS,NSTATIONS_MAX)
        REAL        HHO(NRESER_MAX,DAYS), ENERGYPRO(NRESER_MAX,DAYS), HTK(NRESER_MAX,DAYS)
        ...
        REAL        OP4(NRESER_MAX,DAYS),OP6(NRESER_MAX,DAYS) ! pre-defined time series data
        ...
        REAL        IRRIGATION(NRESER_MAX,DAYS)
        REAL        KFACTOR(NSTATIONS_MAX)
        CHARACTER*10 WTEMP, W_STR,NDAY_SIM_STR,NDAY_SIM_STR_NEXT
        CHARACTER*72 RESFLOWSTRING, RESFLOWSTRING_SIM
        CHARACTER*72 RPATH
        ...
        CHARACTER*100 storage_filename
...
c       Look for reservoirs sequences
        CURRENTYEAR = START_YEAR
        CLEN=INDEX(OUTPATH,' ')-1    ! Character Length for Output Path  
        DO W=1,NSTATIONS
            ...
            IF (STEPBYSTEP) THEN
                DO J = 1,(NORESERVOIRS(W)-1) 
                    OPEN(98,file = OUTPATH(1:CLEN)//'/STEPBYSTEP/'//trim(adjustl(NAMERS5(W,RESORDER(W,J))))//'.uh_s', status='old')
                ...
                J=NORESERVOIRS(W)
                OPEN(97,file = OUTPATH(1:CLEN)//'/STEPBYSTEP/'//trim(adjustl(NAME5(W)))//'.uh_s', status='old')
                DO N= 1,NO_OF_BOX(RESORDER(W,J),W)
            ...
            ELSE ! Not STEPBYSTEP Version
                DO J = 1,(NORESERVOIRS(W)-1) 
                    OPEN(98,file = OUTPATH(1:CLEN)//trim(adjustl(NAMERS5(W,RESORDER(W,J))))//'.uh_s', status='old')
                    DO N = 1,NO_OF_BOX(RESORDER(W,J),W)    
                        READ(98, *) (UH_SS(N,K,RESORDER(W,J)), K = 1,KE+UH_DAY-1)  
                    ENDDO
                    CLOSE(98)
                ...
                J=NORESERVOIRS(W)
                OPEN(97,file = OUTPATH(1:CLEN)//trim(adjustl(NAME5(W)))//'.uh_s', status='old')
                DO N= 1,NO_OF_BOX(RESORDER(W,J),W)
                    READ(97, *) (UH_SS(N,K,RESORDER(W,J)), K = 1,KE+UH_DAY-1)
                ENDDO
                CLOSE(97)
...
c------------------------------------------------------------------------------------------------------------------------------------------------------------------                        
            CLEN=INDEX(OUTPATH,' ')-1    ! Character Length for Output Path (from configuration.txt)    
            WRITE(WTEMP, 10) W
            write(NDAY_SIM_STR,10) NDAY_SIM    
10          FORMAT(I4)
            RESFLOWSTRING = trim(trim('RF'//trim(ADJUSTL(WTEMP)))//'.txt')
            IF (STEPBYSTEP .AND. (NDAY_SIM.GT.1)) THEN
                print*, 'No need to rerun MAKE_CONVOLUTION for Step-By-Step Version'
                print*, 'Read updated naturalized flow from previous step'
                !READ RESFLOWS
                RESFLOWSTRING_SIM = trim(trim('RF'//trim(ADJUSTL(WTEMP)))//'_STEP'//trim(adjustl(NDAY_SIM_STR))//'.txt')
                open(72, FILE=OUTPATH(1:CLEN)//'/STEPBYSTEP/'//RESFLOWSTRING_SIM, status='old')
                DO N = 1, NORESERVOIRS(W)
                    READ(72, *) (RESFLOWS(N,K,W), K = 1, NDAY)
                ENDDO
                !close(72,status='delete')
                close(72)      
            ELSE ! Not STEPBYSTEP Version or STEPBYSTEP with step=1 
                DO N = 1,NORESERVOIRS(W)
     ...
     &                   FLOWIN, FLOWOUT(:,:,W), HHO, RESFLOWS(:,:,W),N,RES_DIRECT(N,1,W),
     &                   RES_EVAPORATION, NORESERVOIRS(W),NRESER_MAX)   
                ENDDO
c               !SAVE RESFLOWS TO READ THEM LATER    
                IF (STEPBYSTEP) THEN
                    RESFLOWSTRING= trim(trim('RF'//trim(ADJUSTL(WTEMP)))//'_STEP'//trim(adjustl(NDAY_SIM_STR))//'.txt')
                    open(71, FILE = OUTPATH(1:CLEN)//'/STEPBYSTEP/'//RESFLOWSTRING, status='unknown')
                ELSE
                    RESFLOWSTRING = trim(trim('RF'//trim(ADJUSTL(WTEMP)))//'.txt')
                    open(71, FILE = OUTPATH(1:CLEN)//RESFLOWSTRING, status='unknown')
                ENDIF
                DO N = 1,NORESERVOIRS(W)
...
c               !READ RESFLOWS
                IF (STEPBYSTEP) THEN
                    open(72, FILE=OUTPATH(1:CLEN)//'/STEPBYSTEP/'//RESFLOWSTRING, status='old',ERR=9004)
                ELSE
                    open(72, FILE=OUTPATH(1:CLEN)//RESFLOWSTRING, status='old',ERR=9004)
                ENDIF
                DO N = 1, NORESERVOIRS(W)
                    READ(72, *) (RESFLOWS(N,K,W), K = 1, NDAY)
                ENDDO
            ...
            IF (STEPBYSTEP) THEN
               ! I = 1,2            ! only one step to run and next step to save I+1 step variables
                ...
                IF (NDAY_SIM.GT.1) THEN
                    ...
                    open(99,FILE = OUTPATH(1:CLEN)//'/STEPBYSTEP/'//storage_filename,STATUS='OLD',ERR=9004)
     ...
     &              RESLV(J,7,W), RESLV(J,8,W), RESLV(J,9,W), RESLV(J,10,W), RESLV(J,11,W), RESLV(J,12,W)
                ...
                ELSE IF (RULE(J,W) .EQ. 4) THEN! predefined time-series of release
                    ...
                    DO L = 1, NO_OF_ROW(J,W)
                        READ(26,*) YEAR(J,L), MONTH(J,L), DAY(J,L), OP4(J,L)
                ...
                ELSE IF (RULE(J,W) .EQ. 6) THEN  !predefined time-series of storage
                    ...
                    DO L = 1, NO_OF_ROW(J,W)
                        READ(27,*) YEAR(J,L), MONTH(J,L), DAY(J,L), OP6(J,L)
                        IF ((YEAR(J,L) .EQ. START_YEAR) .AND. (MONTH(J,L) .EQ. START_MO) .AND. (DAY(J,L) .EQ. 1)) THEN
...
!               H0(J,W) = (HRESERMAX(J,W)-HRESERMIN(J,W)*(VRESERTHAT(J,W)+VDEAD(J,W))/VDEAD(J,W))/(1-(VRESERTHAT(J,W)+VDEAD(J,W))/VDEAD(J,W))    !This actually performs better, but we will fix the problem of H0 with reservoirs bathimetry
                ...
                IF (CURRENTYEAR>=OPEYEAR(J,W)) THEN
                    IF (STEPBYSTEP .AND. (NDAY_SIM.GT.1)) THEN   ! CURRENTYEAR=START_YEAR; do we need to change this condition for the case when NDAY_SIM>1 but CURRENTYEAR>=OPEYEAR, then we need to assign VOL=Vinitial?
...
c xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
                    ...
                    FLOWIN(J,I) = RESFLOWS(J,II,W)     ! change here I (step) to II (simulated day)
c xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx            
...
c xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
     ...
     &                              (CRTDATE-OP1(J,1,W))+(HMAX(J,W)-HMIN(J,W))
                        ...
                        IF (BTHMET_ACTIVE(J,W).EQ.1) THEN
                            CURRENTWL(W)=BTHMET_A(J,W)/(EXP(-1*VOL(J,I,W)/1000*BTHMET_B(J,W))+VOL(J,I,W)*BTHMET_C(J,W)+BTHMET_D(J,W))   ! Fitting Equation by Shanti [convert volume from 1000M3 to MCM]
                        ELSE
...
c xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
                    ELSE IF (RULE(J,W) .EQ. 4) THEN! pre-defined time series (Option 4)
                        IF ((OP4(J,I+STARTDAY(J,W)) .GT. QRESER(J,W)) .AND. (VOL(J,I,W) .LT. VRESER(J,W,I))) THEN
                            OP4(J,I+STARTDAY(J,W)) = QRESER(J,W)
                        END IF
                        IF (OP4(J,I+STARTDAY(J,W))*24*3.6 .GT. ((VOL(J,I,W)-VDEAD(J,W)))+FLOWIN(J,I)*24*3.6) THEN
                            FLOWOUT(J,I,W) = (VOL(J,I,W)-VDEAD(J,W))/24/3.6 + FLOWIN(J,I)
                        ELSE
                            FLOWOUT(J,I,W) = OP4(J,I+STARTDAY(J,W))
                        END IF
                        VOL(J,I+1,W) = VOL(J,I,W) + (FLOWIN(J,I)-FLOWOUT(J,I,W))*24*3.6
                        GOTO 123
c xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx            
...
c xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
                    ELSE IF (RULE(J,W) .EQ. 6) THEN
                        IF (FLOWIN(J,I)-(OP6(J,I+STARTDAY(J,W))-VOL(J,I,W))/24/3.6 >0) THEN
                            VOL(J,I+1,W) = OP6(J,I+STARTDAY(J,W))
                            FLOWOUT(J,I,W) = FLOWIN(J,I)-(VOL(J,I+1,W)-VOL(J,I,W))/24/3.6
                        ELSE
                            FLOWOUT(J,I,W) = 0
                            VOL(J,I+1,W) = VOL(J,I,W) + FLOWIN(J,I)*24*3.6
...
c                   ! Check the neccesity to spill water
                    ...
                    ENERGYPRO(J,I) = ENERGYPRO(J,I) *(( HHO(J,I)+HHO(J,I+1))/2-(HRESERMAX(J,W)-REALHEAD(J,W)))/1000			! this part is for hydropower production estimation, ignore if work with irrigation reservoirs
c xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx            
c     STEP 6: Propagate water to the downstream reservoir, considering the time lag
c xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
                    RESORD(W) = NORESERVOIRS(W)
                    ...
                    IF (RES_DIRECT(J,2,W) .EQ. 0) THEN  ! No downstream reservoirs
                        RESFLOWS(NORESERVOIRS(W),II+1+INT(TRVLTIME(J)),W) = RESFLOWS(NORESERVOIRS(W),II+1+INT(TRVLTIME(J)),W) + FLOWOUT(J,I,W)
                        
                    ELSE
                        RESFLOWS(RESORD(W),II+1+INT(TRVLTIME(J)),W) = RESFLOWS(RESORD(W),II+1+INT(TRVLTIME(J)),W) + FLOWOUT(J,I,W)
...
c                   !SAVE RESFLOWS TO READ THEM LATER for StepByStep Version 
                    RESFLOWSTRING_SIM = trim(trim('RF'//trim(ADJUSTL(W_STR)))//'_STEP'//trim(adjustl(NDAY_SIM_STR_NEXT))//'.txt')
                    open(73, file = OUTPATH(1:CLEN)//'/STEPBYSTEP/'//RESFLOWSTRING_SIM)
                    DO N = 1,NORESERVOIRS(W)
                    ...
                    open(88, file=OUTPATH(1:CLEN)//'/STEPBYSTEP/'//storage_filename)
                    DO J = 1, NORESERVOIRS(W)
                        WRITE(88, *) (VOL(J,I+1,W))
                    END DO
                    close(88)
                END IF
