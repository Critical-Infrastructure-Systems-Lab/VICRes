...
C************************************************************************************************************************************************************************************   
     ...
     &  ICOL, IROW, RPATH, NORESERVOIRS,RES_DIRECT,RES_EVAPORATION,NO_OF_BOX, NRESER_MAX,STEPBYSTEP,NDAY_SIM,SIM_YEAR, SIM_MON, SIM_DAY,COUPLER_ITERATION)
      IMPLICIT NONE
c     Declare variables
c     RCS ID STRING
      INTEGER       NRESER_MAX,NDAY_SIM,SIM_YEAR, SIM_MON, SIM_DAY,COUPLER_ITERATION
      INTEGER       IROW, ICOL
...
c     Write reservoirs output of the last station
     ...
     & (FLOW, DAYS, NAME5, FACTOR_SUM, OUTPATH,IDAY,IMONTH,IYEAR,
     &  VOL, FLOWIN, FLOWOUT,FLOWOUT_TURB, HHO, HYDROPOWER,HTK, RESER, NCOL,NROW,
     &  ICOL, IROW, RPATH, NORESERVOIRS,RES_DIRECT,RES_EVAPORATION,NO_OF_BOX,VRESER, NRESER_MAX,STEPBYSTEP,NDAY_SIM,SIM_YEAR, SIM_MON, SIM_DAY,COUPLER_ITERATION)
      IMPLICIT NONE
c     Declare variables
c     RCS ID STRING
      INTEGER       NSTATIONS_MAX, NRESER_MAX,NDAY_SIM,STEPS,SIM_YEAR, SIM_MON, SIM_DAY,COUPLER_ITERATION
      INTEGER       IROW, ICOL
      INTEGER       NO_OF_BOX(NRESER_MAX)
      INTEGER       DAYS
      INTEGER       NROW, NCOL
      ...
      INTEGER       RULE, IRRIGATION
      INTEGER       NORESERVOIRS, NAM
      REAL          VOL(NRESER_MAX,DAYS),FLOWIN(NRESER_MAX,DAYS),FLOWOUT(NRESER_MAX,DAYS), FLOWOUT_TURB(NRESER_MAX,DAYS),VRESER(NRESER_MAX,DAYS)
      REAL          HHO(NRESER_MAX,DAYS),HYDROPOWER(NRESER_MAX,DAYS),HTK(NRESER_MAX,DAYS)
      REAL          SEEPAGE, INFILTRATION
      REAL          RES_EVAPORATION(NRESER_MAX,DAYS)
      REAL          HRESERMAX, HRESERMIN
...
c     Subroutine main body
     ...
     &                 //trim(ADJUSTL(CHUOI))//'_'//trim(ADJUSTL(NAME5))//'.day')
                        IF ((RULE .EQ. 1) .OR. (RULE .EQ. 2) .OR. (RULE .EQ. 3) .OR. (RULE .EQ. 5) .OR. (RULE .EQ. 6)) THEN
                              WRITE(40,*) 'Year Month Day Volume_1000cm Volume_t+1_1000cm FullSupplyVolume WaterLevel_m WaterLevel_t+1_m Qinflow_cms '
     &                     //'Qoutflow_cms Qturbine_cms Energy_MW'
                        ELSE
                              WRITE(40,*) 'Year Month Day Volume_1000cm Volume_t+1_1000cm FullSupplyVolume WaterLevel_m  WaterLevel_t+1_m Qinflow_cms '
     &                        //'Qoutflow_cms Qturbine_cms Energy_MW'
                        ENDIF            
